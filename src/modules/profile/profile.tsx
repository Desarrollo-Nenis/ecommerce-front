...
interface ProfileProps {
  ...
  orders?: Pedido[] | undefined | null;
}

export function ProfileLayout({ user, userAvatar, orders }: ProfileProps) {
  return (
    <div className="container mx-auto py-6 px-4 md:px-6 lg:px-8 max-w-7xl">
      <div className="grid gap-6 md:grid-cols-5">
        ...
        <div className="md:col-span-3 lg:col-span-4 space-y-6">
          ...
          <ProfileTabs user={user} orders={orders ?? []} />
        </div>
      </div>
    </div>
  );
...
function ProfileTabs({ user, orders }: { user: User | null, orders: Pedido[] | null }) {
  return (
    <Tabs defaultValue="settings" className="w-full">
      ...
      <TabsContent value="information" className="space-y-4">
        <InformacionPersonal compras={""} productsComprados={""} clientTime={user?.createdAt.toString() || ""} orders={orders ?? []} />
      </TabsContent>
    </Tabs>
  );
}
